/* ==========================================================================
   FieldOps Atlas RF Builder 3 — diagnostic surface colours
   File: FieldOpsAtlas/Features/RF/rf-graph-builder-3.js
   Version: 1.1.254-builder-3-diagnostic-surface-colours-13k

   Geometry:
   - One geometry build, one material, one rendered mountain mesh.
   - One visible surface layer and one material only.
   - Diagnostic colours identify geometry components and face types.
   - Red: low near-horizontal faces.
   - Magenta: downward-facing faces.
   - Yellow: near-vertical faces.
   - Other colours: separate connected components.
...
(() => {
  ...
  const VERSION = "1.1.254-builder-3-diagnostic-surface-colours-13k";
  const MODE = "three-neon-peak-diagnostic-surface-colours-builder-3";
  ...
  const OPAQUE_SURFACE_MIN_RGB = Object.freeze([10 / 255, 34 / 255, 42 / 255]);
  ...
  function applyOpaqueColourFloor(red, green, blue) {
    return [
      Math.max(OPAQUE_SURFACE_MIN_RGB[0], red),
      Math.max(OPAQUE_SURFACE_MIN_RGB[1], green),
      Math.max(OPAQUE_SURFACE_MIN_RGB[2], blue)
    ];
  }

  function brightenColour(red, green, blue, influence) {
    if (influence <= 0.001) {
      return applyOpaqueColourFloor(red, green, blue);
    }
  ...
  function triangleNormalAndArea(position, first, second, third) {
    ...
    const inverseLength = areaSquared > MINIMUM_TRIANGLE_AREA_SQUARED
      ? 1 / Math.sqrt(areaSquared)
      : 0;

    return {
      areaSquared,
      normalY: Math.abs(crossY) * inverseLength,
      signedNormalY: crossY * inverseLength
    };
  }

  const DIAGNOSTIC_COMPONENT_COLOURS = Object.freeze([
    Object.freeze([0.05, 0.95, 1.00]),
    Object.freeze([0.15, 1.00, 0.30]),
    Object.freeze([0.20, 0.45, 1.00]),
    Object.freeze([1.00, 0.45, 0.05]),
    Object.freeze([0.55, 0.25, 1.00]),
    Object.freeze([0.15, 0.85, 0.65]),
    Object.freeze([0.90, 0.85, 0.10]),
    Object.freeze([0.35, 0.65, 1.00])
  ]);

  const DIAGNOSTIC_LOW_HORIZONTAL = Object.freeze([1.00, 0.05, 0.05]);
  const DIAGNOSTIC_DOWNWARD = Object.freeze([1.00, 0.05, 0.80]);
  const DIAGNOSTIC_VERTICAL = Object.freeze([1.00, 0.85, 0.05]);

  function computeFaceComponents(index, vertexCount) {
    const faceCount = Math.floor(index.count / 3);
    const parent = new Int32Array(faceCount);
    const rank = new Uint8Array(faceCount);
    const firstFaceByVertex = new Int32Array(vertexCount);
    firstFaceByVertex.fill(-1);
    ...
    function union(firstFace, secondFace) {
      let firstRoot = find(firstFace);
      let secondRoot = find(secondFace);
    ...
    for (let face = 0; face < faceCount; face += 1) {
      ...
      for (let corner = 0; corner < 3; corner += 1) {
        const vertexIndex = index.getX(offset + corner);

        if (vertexIndex < 0 || vertexIndex >= vertexCount) continue;

        const existingFace = firstFaceByVertex[vertexIndex];

        if (existingFace < 0) {
          firstFaceByVertex[vertexIndex] = face;
        } else {
          union(face, existingFace);
        }
      }
    }

    const componentIds = new Int32Array(faceCount);
    ...
    for (let face = 0; face < faceCount; face += 1) {
      ...
      if (!componentLookup.has(root)) {
        componentLookup.set(root, componentLookup.size);
        componentSizes.push(0);
      }

      const componentId = componentLookup.get(root);
      componentIds[face] = componentId;
    ...
    return {
      componentIds,
      componentSizes,
      componentCount: componentLookup.size
    };
  }

  function diagnosticFaceColour(triangle, lowHorizontalFace, componentId) {
    if (lowHorizontalFace) {
      return DIAGNOSTIC_LOW_HORIZONTAL;
    }

    if (triangle.signedNormalY < -0.18) {
      return DIAGNOSTIC_DOWNWARD;
    }

    if (triangle.normalY < 0.14) {
      return DIAGNOSTIC_VERTICAL;
    }

    return DIAGNOSTIC_COMPONENT_COLOURS[
      componentId % DIAGNOSTIC_COMPONENT_COLOURS.length
    ];
  }
  ...
  function buildSingleLayerGeometry(THREE, sourceGeometry) {
    ...
    const componentData = computeFaceComponents(index, position.count);
    const positions = [];
    const colours = [];

    let retainedFaceCount = 0;
    let removedInvalidFaceCount = 0;
    let diagnosticLowHorizontalFaceCount = 0;
    let diagnosticDownwardFaceCount = 0;
    let diagnosticVerticalFaceCount = 0;
    ...
    for (let offset = 0; offset < index.count; offset += 3) {
      ...
      const lowHorizontalFace = (
        faceMaximumY <= horizontalMaximumY
        && triangle.normalY >= HORIZONTAL_NORMAL_Y_MIN
      );
      const faceIndex = offset / 3;
      const componentId = componentData.componentIds[faceIndex];
      const faceColour = diagnosticFaceColour(
        triangle,
        lowHorizontalFace,
        componentId
      );
      ...
      if (lowHorizontalFace) {
        diagnosticLowHorizontalFaceCount += 1;
      } else if (triangle.signedNormalY < -0.18) {
        diagnosticDownwardFaceCount += 1;
      } else if (triangle.normalY < 0.14) {
        diagnosticVerticalFaceCount += 1;
      }
    ...
    geometry.userData.rfDiagnosticComponentCount = (
      componentData.componentCount
    );
    geometry.userData.rfDiagnosticComponentSizes = (
      componentData.componentSizes.slice()
    );
    geometry.userData.rfDiagnosticLowHorizontalFaceCount = (
      diagnosticLowHorizontalFaceCount
    );
    geometry.userData.rfDiagnosticDownwardFaceCount = (
      diagnosticDownwardFaceCount
    );
    geometry.userData.rfDiagnosticVerticalFaceCount = (
      diagnosticVerticalFaceCount
    );
  ...
  function createSingleLayerMaterial(THREE) {
    ...
    material.name = "rf-diagnostic-surface-colours-material";
    return material;
  }
  ...
  function createMountain(THREE) {
    ...
    mountain.name = "rf-diagnostic-surface-colours-mesh";
  ...
  window.FieldOpsRFGraph = { VERSION, META, renderMeshes: 1, visibleLayers: 1, diagnosticColours: true, init: initialiseMount, initAll };
