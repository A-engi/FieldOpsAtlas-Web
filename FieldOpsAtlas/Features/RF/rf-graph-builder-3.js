/* ==========================================================================
   FieldOps Atlas RF Builder 3 — two colour layers with embedded triangle edges
   File: FieldOpsAtlas/Features/RF/rf-graph-builder-3.js
   Version: 1.1.304-builder-3-two-colour-layers-triangle-edges

   Geometry:
   - Retains the approved outer mountain surface and all current colour layers.
   - Removes the duplicated inward triangle shell that caused overlapping faces.
   - Renders both triangle windings with full depth writing so mixed-winding source faces keep their colour.
   - Keeps one closed dark core behind genuine openings in the non-watertight source mesh.
   - Keeps exactly two visible colour layers: the main summit and the secondary shoulders/forks.
   - Embeds triangle-edge colour inside those two layers only; removes the global border and rim layers.
   - Keeps the centre peak, secondary shoulders, fork sectors and camera behaviour unchanged.
   ========================================================================== */
(() => {
  "use strict";

  const VERSION = "1.1.304-builder-3-two-colour-layers-triangle-edges";
  const MODE = "three-neon-peak-two-colour-layers-triangle-edges-builder-3";
  ...
  function buildSingleLayerGeometry(THREE, sourceGeometry) {
    ...
    geometry.userData.rfIntegratedOcclusionFaceCount = 0;
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float triangleBorder = triangleEdgeMask();
          float layer1Presence = clamp(
            max(layer1Area, max(layer1Band, layer1Vein)),
            0.0,
            1.0
          );
          float layer1Edge = triangleBorder * layer1Presence;
          float layer2Edge = triangleBorder * layer2Presence;

          /*
           * The edge colour belongs to the two approved colour layers.
           * There is no third global border layer and no separate rim layer.
           */
          colour = mix(colour, uOutline, layer1Edge * 0.94);
          colour = mix(colour, uAccent, layer2Edge * 0.92);
          colour += uGlow * max(layer1Edge, layer2Edge) * 0.12;

          gl_FragColor = vec4(colour, 1.0);
        }
      `
    });
    material.name = "rf-two-colour-layers-triangle-edges-outer-material";
    return material;
  }

  function createOpaqueCore(THREE, outerGeometry) {
    outerGeometry.computeBoundingBox();
    const bounds = outerGeometry.boundingBox;

    if (!bounds) {
      throw new Error("Builder 3 outer bounds are unavailable.");
    }
    ...
    core.name = "rf-solid-opaque-core";
    core.position.set(
      center.x,
    ...
    core.renderOrder = -1;
    core.userData.rfOpaqueCore = true;
    core.userData.rfOpaqueCoreVersion = VERSION;
    return core;
  }
  ...
  function createMountain(THREE) {
    ...
    outer.name = "rf-two-colour-layers-triangle-edges-outer-mesh";
    outer.renderOrder = 0;
    outer.userData.rfTwoLayerTopologyForks360Mesh = true;
    outer.userData.rfTwoLayerTopologyForks360MeshVersion = VERSION;

    const root = new THREE.Group();
    root.name = "rf-two-colour-layers-triangle-edges-mountain";
    root.add(createOpaqueCore(THREE, geometry));
    root.add(outer);
    root.userData.rfTwoLayerTopologyForks360Mesh = true;
    root.userData.rfTwoLayerTopologyForks360MeshVersion = VERSION;
    root.userData.rfOpaqueCore = true;
    return root;
  }
  ...
  window.FieldOpsRFGraph = { VERSION, META, renderMeshes: 2, init: initialiseMount, initAll };
