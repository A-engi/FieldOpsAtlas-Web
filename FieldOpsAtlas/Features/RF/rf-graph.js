  ...
  function createTerrain() {
    ...
    for (let row = 0; row < rows; row += 1) {
      ...
      for (let column = 0; column < columns; column += 1) {
        ...
        points.push(x, y + 0.018, z);
        const glow = clamp((y + 1.2) / 7.2, 0, 1);
        pointColors.push(0.02, 0.42 + glow * 0.24, 0.52 + glow * 0.34, 0.48);
      }
    }
    ...
    function pushTriangle(a, b, c) {
      [a, b, c].forEach((point) => {
        ...
        const glow = clamp((point[1] + 1.2) / 7.2, 0, 1);
        vertexColors.push(0.005, 0.08 + glow * 0.08, 0.12 + glow * 0.12, 0.74);
      });
    }

    const lineVertices = [];
    const lineColors = [];
    function pushLine(a, b, alpha = 0.58) {
      lineVertices.push(a[0], a[1] + 0.012, a[2], b[0], b[1] + 0.012, b[2]);
      const glowA = clamp((a[1] + 1.2) / 7.2, 0, 1);
      const glowB = clamp((b[1] + 1.2) / 7.2, 0, 1);
      lineColors.push(0.01, 0.62 + glowA * 0.25, 0.72 + glowA * 0.25, alpha);
      lineColors.push(0.01, 0.62 + glowB * 0.25, 0.72 + glowB * 0.25, alpha);
    }
    ...
    for (let row = 0; row < rows - 1; row += 1) {
      pushLine(grid[row][columns - 1], grid[row + 1][columns - 1], 0.42);
    }
    for (let column = 0; column < columns - 1; column += 1) {
      pushLine(grid[rows - 1][column], grid[rows - 1][column + 1], 0.42);
    }
